...
-- This script will only insert data if the tables are empty to prevent duplicate data

-- Check if Series table has data, if not, insert seed data
...
-- Check if Species table has data, if not, insert seed data
IF NOT EXISTS (SELECT 1 FROM dbo.Species)
BEGIN
...
-- Check if Actor table has data, if not, insert seed data
-- Note: Using IDENTITY_INSERT for Actor table since it has IDENTITY column
IF NOT EXISTS (SELECT 1 FROM dbo.Actor)
BEGIN
...
-- Check if Character table has data, if not, insert seed data
...
-- Check if Series_Character table has data, if not, insert seed data
IF NOT EXISTS (SELECT 1 FROM dbo.Series_Character)
BEGIN
...
-- Check if Character_Species table has data, if not, insert seed data
IF NOT EXISTS (SELECT 1 FROM dbo.Character_Species)
BEGIN
