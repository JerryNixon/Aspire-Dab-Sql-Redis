...
public class MetadataService : IMetadataService
{
    ...
    public async Task<ConnectionTestResult> TestConnectionAsync(string connectionString)
    {
        try
        {
            ...
            _logger.LogInformation("Connection test successful: {Database} on {Server}", database, connection.DataSource);
    ...
    {
        _logger.LogInformation("Loading database metadata");
    ...
    private async Task<List<TableMetadata>> GetTablesAsync(SqlConnection connection)
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            current!.Columns.Add(new ColumnMetadata
            {
    ...
    private async Task<List<ViewMetadata>> GetViewsAsync(SqlConnection connection)
    {
    ...
    private async Task<List<StoredProcedureMetadata>> GetStoredProceduresAsync(SqlConnection connection)
    {
        var procedures = new List<StoredProcedureMetadata>();

        // Get procedures and their parameters
        const string sql = @"
            SELECT 
                s.name AS SchemaName,
        ...
        while (await reader.ReadAsync())
        {
            ...
            if (!reader.IsDBNull(2)) // Has parameter
            {
                current!.Parameters.Add(new ParameterMetadata
                {
                    Name = reader.GetString(2),
                    DataType = reader.GetString(3),
        ...
        // Get output columns for each procedure using SET FMTONLY ON
        foreach (var proc in procedures)
        {
            try
            {
                proc.OutputColumns.AddRange(await GetProcedureOutputColumnsAsync(connection, proc.FullName));
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to get output columns for {Procedure}", proc.FullName);
            }
        }
    ...
    private async Task<List<ColumnMetadata>> GetProcedureOutputColumnsAsync(SqlConnection connection, string procedureName)
    {
        var columns = new List<ColumnMetadata>();

        try
        {
            // Use sp_describe_first_result_set to get output columns
            await using var command = new SqlCommand("sp_describe_first_result_set", connection);
            ...
            while (await reader.ReadAsync())
            {
                var columnName = reader.IsDBNull(reader.GetOrdinal("name")) 
                    ? $"Column{reader.GetInt32(reader.GetOrdinal("column_ordinal"))}" 
                ...
                {
                    Name = columnName,
                    DataType = systemTypeName,
                    IsNullable = isNullable,
                    MaxLength = reader.IsDBNull(reader.GetOrdinal("max_length")) ? null : reader.GetInt16(reader.GetOrdinal("max_length")),
        ...
        {
            _logger.LogWarning(ex, "Failed to get output columns for {Procedure} using sp_describe_first_result_set", procedureName);
        }
