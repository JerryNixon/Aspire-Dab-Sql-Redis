...
public class IndexModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostTestConnectionAsync([FromBody] AppSettings settings)
    {
        try
        {
            ...
            if (settings == null)
            {
                _logger.LogError("Settings object is null");
                return BadRequest(new { success = false, errorMessage = "Invalid settings data" });
            }
            
            if (string.IsNullOrWhiteSpace(settings.Server))
            {
                _logger.LogError("Server is null or empty");
                return BadRequest(new { success = false, errorMessage = "Server is required" });
            }
        ...
        catch (Exception ex)
        {
            ...
            return new JsonResult(new ConnectionTestResult
            {
                Success = false,
    ...
    public async Task<IActionResult> OnGetMetadataAsync()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(connectionString) || string.IsNullOrWhiteSpace(settings.Server))
            {
    ...
    public async Task<IActionResult> OnPostExecuteQueryAsync([FromBody] QueryRequest request)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(connectionString) || string.IsNullOrWhiteSpace(settings.Server))
            {
                return new JsonResult(new QueryResponse
                {
                    Success = false,
